...
<div class="d-lg-flex mx-auto justify-content-center w-100 mt-9 p-2" style="gap: 1rem;">
    ...
    <div class="gap-10" style="width: 650px;">
            ...
            @foreach($notifications as $index => $notif)
                ...
                @endphp
                    ...
                    <div class="d-flex gap-2 py-4 px-4 {{$isNew ? 'bg-lightPrimary2 border-dark' : 'bg-white'}} {{$index > 0 ? 'border-top' : ''}} {{$loop->first ? 'rounded-top' : ''}} {{$loop->last ? 'rounded-bottom' : ''}}">
                        <img src="{{asset('IMG/uploads/profile/' . $notif->sender->profile_image)}}" class="rounded-circle" style="width: 45px; height:45px; object-fit:cover;">
                        <div class="d-block w-100">
                            {!! '<div class="fs-8 text-truncate-2">'.$notif->title.' '.$notif->created_at->diffForHumans().'</div>' !!}
                            <div class="{{$isNew ? 'bg-white' : 'bg-main'}} rounded text-truncate-1 mb-2 fs-8 px-2 w-100">
                                <p class="mb-0 py-1 w-100">{{$notif->description}}</p>
                            </div>
                            <a href="{{ route('message.page', ['active_tab' => $notif->sender->user_id]) }}"
                                class="btn btn-outline-primary rounded-pill px-5 pt-0 pb-1 fs-7">
                                Reply Message
                            </a>
                        </div>
